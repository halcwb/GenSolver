...
#load @".paket\load\netcoreapp3.0\Test\test.group.fsx"

#load @"src\Informedica.GenSolver.Lib\Types.fs"
#load @"src\Informedica.GenSolver.Lib\Utils.fs"
#load @"src\Informedica.GenSolver.Lib\Logging.fs"
...
|> Test.filter (String.contains tn)
|> runTests { defaultConfig with verbosity = Logging.LogLevel.Verbose }

// [3..>
let vr1 = 
    ValueRange.createMinIncl 3N
    |> fun vr -> Variable.ValueRange.create None (Some vr) None None

// <..2]
let vr2 = 
    ValueRange.createMaxIncl 2N
    |> fun vr -> Variable.ValueRange.create None None None (Some vr)
...
let testDto =
    fun vs min minincl incr max maxincl ->
        ...
        printfn "created dto: %A" dto
        match dto |> DTO.fromDtoOpt with
        | Some vr -> 
            try
                let dto'  = vr |> toDto |> fromDto |> Option.get |> toDto
                let dto'' = dto' |> fromDto |> Option.get |> toDto
                printfn "Passed: dto: %s vr: _" (dto'' |> DTO.toString true) 
                dto' = dto''   
            with
            | _ -> printfn "Failed dto: %A vr: %A toDto:%A" dto vr (vr |> toDto); false
        | None -> 
            printfn "Dto: %A %s cannot be parsed" dto (dto |> DTO.toString true)
            true

testDto [] None false [1N; 0N] None false
