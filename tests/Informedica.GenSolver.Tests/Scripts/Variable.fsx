...
#load @"src\Informedica.GenSolver.Lib\Utils.fs"
#load @".\src\Informedica.GenSolver.Lib\Variable.fs"
#load @".\src\Informedica.GenSolver.Lib\Equation.fs"
#load @".\src\Informedica.GenSolver.Lib\Dtos.fs"
    ...
    let tests =
        testList "Variable Name Tests" [
            ...
            fun s -> 
                ...
                let fail = function 
                    | NullOrWhiteSpaceException -> true
                    | LongerThan1000 x -> x > 1000
...
module ValueRange =
    ...
    let getMin  = getMin >> Option.bind (minToValue >> Some)
    let getMax  = getMax >> Option.bind (maxToValue >> Some)

    let createMinIncl = createMin true
    let createMinExcl = createMin false
    let createMaxIncl = createMax true
    let createMaxExcl = createMax false

    let contains v vr = vr |> contains v

    let isBetweenMinMax min max  = isBetweenAndMultOf min None max 

    let createExcMinMax vs min max = createExc false vs min None max
    let createMinMax succ fail vs min max = create succ fail false vs min None max
    ...
    let tests =
        ...
        testList "ValueRange" [
            ...
            testList "Given Min is None Incr is None and Max is None" [
                ...
                fun _ ->
                    let succ vr = test <@ vr |> getMin = None && vr |> getMax = None @>
                ...
                fun _ ->
                    ...
                    let fail _  = test <@false@>
                    createMinMax succ fail vs min max
                |> testCase "Creating a ValueRange returns an empty ValueRange"        
    
                let succ vr = test <@vr |> count = 0@>
                let fail _  = test <@false@>
                ...
                fun  x -> 
                    ...
                    vr |> contains x |> not
                |> testProp "The ValueRange cannot contain any Value"
            ]
            ...
            testList "Given one Value Min is None Incr is None Max is None" [
                ...
                fun _ ->
                    let succ vr = test <@ vr |> getMin = Some v && vr |> getMax = Some v @>
                    let fail _  = test <@ false @>
                    createMinMax succ fail vs min max 
                |> testCase "Both Min and Max are the Inclusive and that Value"
    
                fun _ ->
                    let succ vr = test <@ vr |> count = 1@>
                    let fail _  = test <@false@>
                    createMinMax succ fail vs min max
            ...
            testList "Given a ValueRange with Min Incl is 1 and Max is None" [
                ...
                fun _ ->    
                    ...
                    test <@ min |> Option.get |> minSTEmin minExcl @>                     // min incl 1 < min incl 1
                    test <@ min |> Option.get |> minSTEmax maxExcl  @>                    // min incl 1 < max incl 2
                    test <@ min |> Option.get |> minLTmax (1N |> createMaxExcl) @>        // min incl 1 > max incl 1

                    test <@ min |> Option.get |> minLTmax (1N |> createMaxIncl) |> not @> // min incl 1 > max incl 1 is false
                    test <@ min |> Option.get |> minLTmin (1N |> createMinIncl) |> not @> // min incl 1 > min incl 1 is false
            ...
            testList "Given a calculation with ValueRange with a Min and a ValueRange with a Max" [
                ...
                fun v1 v2 incl1 incl2 ->
                    let predMin v1 v2 _ _ m =
                        if v2 > 0N then m = None
                            ...
                            m |> Option.get = (v1 / v2 |> VR.createMin (incl1 && incl2))
                ...
                fun _ ->
                    ...
                    test<@ vr |> VR.getMin = None @>
                    test<@ vr |> VR.getMax = None @>
            ...
            testList "Given addition multiplication or division of two Value Sets" [
